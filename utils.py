...
def is_element_exist(driver, text):
    ...
    # 显示等待查找元素,如果能找到元素则返回元素对象,找不到则返回False,并且抛出异常信息
    try:
...
def is_el_by_attribute(driver, attr_name, attr_value):
    ...
    try:
        ...
        logging.error(NoSuchElementException("找不到属性={}且属性值为{}的元素对象".format(attr_name, attr_value)))
        return False
...
def check_channel_option(driver, channel_name, option_name):
    # 选择框的xpath表达式
    srt_xpath = "//*[contains(@placeholder,'{}')]".format(channel_name)
    # 1.点击选择框的元素对象
    driver.find_element_by_xpath(srt_xpath).click()
    # # 2.选择频道选项并点击
    # self.pub_ari_page.find_channel_option().click()

    # 2.获取所有选项的频道名称
    channel_option = driver.find_elements_by_css_selector(".el-scrollbar__view span")
    ...
    for option_element in channel_option:
        ...
        if option_element.text == option_name:
            # 如果相等则点击,跳出,并且把默认标识改为True
            option_element.click()
            is_suc = True
            break
        # 如果不相等则,鼠标悬浮到当前遍历的元素对象上并按下向下的按钮
        else:
            # 创建鼠标对象
            action = ActionChains(driver)
            action.move_to_element(option_element).send_keys(Keys.DOWN).perform()

    # 判断标识是否为False,则抛出没有找到对应的频道的选项
    if is_suc is False:
...
# allure截图公用方法
def get_allure_png(driver, filname):
    allure.attach(driver.get_screenshot_as_png(), filname, allure.attachment_type.PNG)
